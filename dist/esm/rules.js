            ...
            for (const rule of checkRules) {
                ...
                if (typeof rule === 'string' || typeof rule === 'function') {
                    try {
                        ...
                        const ruleEnv = {
                            ...env,
                            exists: async (target) => this.db.ref(getFullPath(currentPath, target)).exists(),
                            value: async (target, include) => {
        ...
        if (isAllowed && ['set', 'update'].includes(operation) && !isPreFlight) {
            ...
            for (const check of checkRules) {
                ...
                const validateEnv = {
                    ...
                    data: validateData,
                    exists: async (target) => this.db.ref(getFullPath(validatePath, target)).exists(),
                    value: async (target, include) => {
